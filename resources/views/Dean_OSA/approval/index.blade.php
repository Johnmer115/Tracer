...
<section class="panel" style="padding: 25px;">
    ...
    */
    $approvalFields = [
        ...
        ['col' => 'approval_vp_comptroller','rem' => 'remarks_vp_comptroller',  'office' => 'Auditing','role' => 'VP / Comptroller'],
        ['col' => 'approval_avp_finance',   'rem' => 'remarks_avp_finance',    'office' => 'Finance', 'role' => 'Asst VP for Finance'],
    ];
    ...
    |--------------------------------------------------------------------------
    */
    $getApprovalLocation = function($activity) use ($approvalFields) {
        foreach ($approvalFields as $sig) {
        ...
        <div class="panel-header">
            ...
            <form method="GET" action="{{ route('dean_osa.approval.index') }}" class="panel-controls">
                <select name="status" class="filter-select" onchange="this.form.submit()">
                    ...
                    @foreach(['pending','ongoing','for approval','for approval finance','approved','for revision','cancelled'] as $s)
        ...
        <div style="display:flex; gap:8px; flex-wrap:wrap; padding:14px 20px; border-bottom:1px solid #e5e7eb;">
            @php
                $chipData = [
                    ...
                    ['label'=>'For Approval','status'=>'for approval',         'bg'=>'#eff6ff','color'=>'#014ea8','border'=>'#93c5fd'],
                    ['label'=>'Finance',     'status'=>'for approval finance', 'bg'=>'#eff6ff','color'=>'#014ea8','border'=>'#93c5fd'],
            ...
            <table>
                ...
                <tbody>
                    @forelse($activities as $activity)
                        @php
                            $dotStatuses    = collect($approvalFields)->map(fn($s) => $activity->{$s['col']} ?? 'pending');
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                @php $levels = is_array($activity->level) ? $activity->level : []; @endphp
                                @if(count($levels))
                            ...
                            <td style="min-width:170px;">
                                ...
                                <div style="display:flex; align-items:center; gap:5px; flex-wrap:nowrap;">
                                    @foreach($approvalFields as $i => $sig)
                                        @php
                                            $val = $activity->{$sig['col']} ?? 'pending';

                                            // locked = previous not approved AND not yet started
                                            $prevApproved = $i === 0 || ($activity->{$approvalFields[$i-1]['col']} ?? 'pending') === 'approved';
